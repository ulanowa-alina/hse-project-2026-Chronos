...
auto handleDelete(const http::request<http::string_body>& req, ConnectionPool& pool,
    ...
    if (req.method() != http::verb::delete_) {
        spdlog::error("Board delete rejected: method not allowed");
    ...
    try {
        ...
        spdlog::error("Board delete rejected: invalid JSON format");
    ...
    try {
        ...
        if (!board.has_value()) {
            spdlog::error("Board delete rejected: board with id = {} not found", board_id);
            return server::utils::build_error_response(req, http::status::not_found,
                                                       "BOARD_NOT_FOUND", "Board not found");
        }

        if (board->user_id_ != user_id) {
            spdlog::error("Board delete rejected: board with id = {} belongs to another user",
                          board_id);
        ...
        if (message.rfind("missing:", 0) == 0) {
            spdlog::error("Board delete rejected: missing required fields");
        ...
        if (message.rfind("type:", 0) == 0) {
            spdlog::error("Board delete rejected: invalid filed format");
        ...
        if (message.rfind("value:", 0) == 0) {
            spdlog::error("Board delete rejected: invalid field value");
