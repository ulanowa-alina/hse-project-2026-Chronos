...
auto handleEdit(const http::request<http::string_body>& req, ConnectionPool& pool,
    ...
    if (req.method() != http::verb::patch) {
        spdlog::warn("Task edit rejected: method not allowed");
    ...
    try {
        ...
        spdlog::warn("Task edit rejected: invalid JSON format");
    ...
    if (!missing_fields.empty()) {
        spdlog::warn("Task edit rejected: missing required fields");
    ...
    try {
        ...
        if (title.empty() || title.size() > 100) {
            spdlog::warn("Task edit rejected: invalid title length");
        ...
        if (description.size() > 1000) {
            spdlog::warn("Task edit rejected: description is too long");
            return server::utils::build_error_response(
                req, http::status::bad_request, "VALIDATION_ERROR", "Validation failed",
        ...
        if (priority_color.empty() || priority_color.size() > 50) {
            spdlog::warn("Task edit rejected: invalid priority color format");
            return server::utils::build_error_response(
                req, http::status::bad_request, "VALIDATION_ERROR", "Validation failed",
        ...
        if (!existing_task.has_value()) {
            spdlog::warn("Task edit rejected: task with id={} not found", task_id);
        ...
        if (task_board->user_id_ != user_id) {
            spdlog::warn("Task edit rejected: board with id={} belongs to another user",
                         existing_task->board_id_);
        ...
        if (!new_status.has_value()) {
            spdlog::warn("Task edit rejected: status with id={} not found", status_id);
            return server::utils::build_error_response(req, http::status::not_found,
                                                       "STATUS_NOT_FOUND", "Status not found");
        }

        if (new_status->board_id_ != existing_task->board_id_) {
            spdlog::warn("Task edit rejected: invalid status_id");
            return server::utils::build_error_response(
                req, http::status::bad_request, "VALIDATION_ERROR", "Validation failed",
        ...
        if (message.rfind("type:", 0) == 0) {
            spdlog::warn("Task edit rejected: invalid field format");
        ...
        if (message.rfind("value:", 0) == 0) {
            spdlog::warn("Task edit rejected: invalid field value");
