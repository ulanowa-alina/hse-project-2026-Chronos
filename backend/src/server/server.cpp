...
Server::Server(asio::io_context& ioc, const std::string& host, unsigned short port,
    ...
    router_["/board/v1/create"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::post) {
                return board::v1::handleCreate(req, pool_, user_id);
            }
    ...
    router_["/board/v1/get_all"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::get) {
                return board::v1::handleGetAll(req, pool_, user_id);
            }
    ...
    router_["/board/v1/tasks"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::get) {
                return board::v1::handleTasks(req, pool_, user_id);
            }
    ...
    router_["/board/v1/get"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::get) {
                return board::v1::handleGet(req, pool_, user_id);
            }
    ...
    router_["/board/v1/delete"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::delete_) {
                return board::v1::handleDelete(req, pool_, user_id);
            }
    ...
    router_["/board/v1/edit"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::patch) {
                return board::v1::handleEdit(req, pool_, user_id);
            }
    ...
    router_["/task/v1/get_all"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::get) {
                return task::v1::handleGetAll(req, pool_, user_id);
            }
    ...
    router_["/status/v1/edit"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::patch) {
                return status::v1::handleEdit(req, pool_, user_id);
            }
            http::response<http::string_body> res{http::status::method_not_allowed, req.version()};
            res.set(http::field::content_type, "application/json");
            res.set(http::field::access_control_allow_origin, "*");
    ...
    router_["/status/v1/delete"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::delete_) {
                return status::v1::handleDelete(req, pool_, user_id);
            }
    ...
    router_["/pomodoro/v1/create"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::post) {
                return pomodoro::v1::handleCreate(req, pool_, user_id);
            }
    ...
    router_["/pomodoro/v1/get_user_sessions"] =
        auth::with_auth([this](const http::request<http::string_body>& req, int user_id) {
            if (req.method() == http::verb::get) {
                return pomodoro::v1::handleGetUserSessions(req, pool_, user_id);
            }
